...
3) each value was normalized (i.e. removed the average of the pattern and divided by the standard deviation)""")
    ...
    fig = px.scatter(x=components[:, 0], y=components[:, 1], color=model.labels_, text=df.index, labels={
                     "x":"First Principal Component",
                     "y":"Second Principal Component",
                 })
    fig.update_traces(textposition='middle right', showlegend=False, marker_coloraxis=None, marker=dict(size=12),
                      textfont_size=8)

...
RETURN g
    ...
    st.write(
        "It returns all the Euro2020 matches in which Donnarumma played against Saka. In the next sections we will see the main ingredients to get to write this query."
    )
...
"""
        ...
        st.write(
            "The data shown are taken from the matches.json file of Statsbomb, taking into account the constraints of neo4j (for example the properties can be atomic types (float, integer, strings...) or lists, but not dictionaries)"
        )
...
"jersey_number": 17
        ...
        st.warning(
            """In the original json file the names contain accented letters and special characters that are not accepted by the database. Therefore all names have been "Englishified". For example, Kylian Mbappé was transformed into Kylian Mbappe"""
        )
...
RETURN a,s,e
        ...
        st.write(
            "While to exclude the penalty kicks it is necessary to remember that StatsBomb encodes the penalties with period=5 "
...
RETURN a,s,e
    ...
    st.write(
        "The other key piece are the relationships, which connect the nodes together."
    )

    with st.expander("HAS_PLAYED, relationship between players and games"):
        st.write(
            """In the nodes, the players have as properties only general information
...
"height": "Ground Pass"
        ...
        st.write(
            """For example, order is used to identify successive steps, while possession_length is used to determine
...
RETURN p
    ...
    st.write(
        "In addition, course-specific functions have been defined to facilitate pattern searches."
    )
    with st.expander("User defined functions for pattern matching"):
        ...
        st.write(
            """This is to check that the players are actually different. 
        ...
        st.warning(
            "Be careful not to put sa.differentPlayers([A,B,A,B,C,A]) which will always return false"
        )
        ...
        st.write(
            "So for example, to match all ABABCA in Euro2020 we need to write te following query"
        )
        st.code(
            """

...
The graphs are interactive. For example, if you click on ABA, it shows you the graph with ABA as the starting pattern (thus removing its "brothers" ABC and AB-lost).
    ...
    fp = open(
        os.path.join(os.curdir, "../data", "sunburst", "sunburst_unconstrained.json"),
        "r",
    )

    data = json.load(fp)
    fp = open(
        os.path.join(os.curdir, "../data", "sunburst", "sunburst_beginning.json"), "r"
    )
    ...
    fig = px.sunburst(names=patterns, parents=prefixes, values=values, color=prefixes)
    st.plotly_chart(fig)
    ...
    figb = px.sunburst(
        names=patternsb, parents=prefixesb, values=valuesb, color=prefixesb
    )
